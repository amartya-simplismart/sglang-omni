...
def test_metrics_exposes_prometheus_payload() -> None:
    health_status = {
        "worker-a:8101": 200,
        "worker-b:8102": 500,
    }
    ...
    body = response.text
    assert "sglang_omni_router_workers{state=\"healthy\"} 1.0" in body
    assert "sglang_omni_router_workers{state=\"unhealthy\"} 1.0" in body
    assert "sglang_omni_router_workers{state=\"routable\"} 1.0" in body
    assert "sglang_omni_router_active_requests{worker=\"worker-a:8101\"} 0.0" in body
